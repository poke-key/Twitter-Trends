...
def tweet_text(tweet):
    return tweet['text'] #returns the text part of the tweet, as a string

    """Return a string, the words in the text of a tweet."""
    "*** YOUR CODE HERE ***"

def tweet_time(tweet):
    return tweet['time'] #returns the time of the tweet
    """Return the datetime representing when a tweet was posted."""
    "*** YOUR CODE HERE ***"

def tweet_location(tweet):
    return make_position(tweet['latitude'],tweet['longitude'])#calls make position function and gets the tweets latitude and longitude
...
def extract_words(text):
    ...
    """
    "*** YOUR CODE HERE ***"
    charlist = list(text) #convert incoming text to a list of characters
    acharlist = [ ch if ch in ascii_letters else " " for ch in charlist ] #filter out all ascii_letters
    text = ''.join(acharlist) #join all characters to form a string of words
    return text.split() #split the string to a list of words and return
...
def has_sentiment(s):
    ...
    "*** YOUR CODE HERE ***"
    if s is None: #checks if sentiment is null
        return False
    return True #returns true if not null
...
def analyze_tweet_sentiment(tweet):
    ...
    """
    "*** YOUR CODE HERE ***"
    #declare two variables to keep sum of sentiments and count of sentiments
    sentiment_sum = 0
    sentiment_count = 0
    # get words from tweet and check if word has sentiments and create a sentient sum
    for word in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(word)):
            sentiment_sum += sentiment_value(get_word_sentiment(word))
            sentiment_count += 1
    if sentiment_count != 0: # return average sentiment or else return None
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    #initialize variables for the total area, and the (x,y) coordinates of the center
    areatotal = 0
    centroidx = 0
    centroidy = 0
    #reads through all of the vertices of the polygon
    for i in range(len(polygon)-1):
    #sets x and y coordinates of polygon
        xa = latitude(polygon[i])
        xb = latitude(polygon[i+1])
        ya = longitude(polygon[i])
        yb = longitude(polygon[i+1])
        #calculates area of polygon
        areatotal += xa*yb - xb*ya
        #calculates center of polygon, used the equation from wikipedia article provided above
        centroidx += (xa + xb)*(xa*yb - xb*ya)
        centroidy += (ya + yb)*(xa*yb - xb*ya)
    area = areatotal /2
    if area == 0:
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    #calls the find_centroid function and calculates it for a state, which is multiple polygons
    total_x, total_y, total_area = 0,0,0
    for polygon in polygons:
        centx, centy, area = find_centroid(polygon)
        total_x += centx * area 
        total_y += centy * area 
        total_area += area
...
def group_tweets_by_state(tweets):
    ...
    state_to_tweets = {}
    for tweet in tweets:
        # go over each state and find to which state center is the tweet closests too
        shortest_distance = 99999999
        shortest_state = ""
        for state in us_states.keys():
            how_far= geo_distance(find_state_center(us_states[state]), tweet_location(tweet))
            if how_far < shortest_distance:
                shortest_distance = how_far 
                shortest_state = state
        
        # find if the state exists just append the tweet, or else create a new state
        if shortest_state in state_to_tweets.keys():
            state_to_tweets[shortest_state].append(tweet)
        else:
            state_to_tweets[shortest_state] = [tweet]
    return state_to_tweets
...
def average_sentiments(tweets_by_state):
    ...
    #returns a dictionary of state mapped to its average sentiment
    average_sentiments_by_state = {}  
    for state in tweets_by_state.keys():
        ...
        for tweet in tweets_by_state[state]:
            if has_sentiment(analyze_tweet_sentiment(tweet)):
                ...
                count += 1
        if count != 0:
            average_sentiments_by_state[state] = total / count
    return average_sentiments_by_state
...
def draw_state_sentiments(state_sentiments):
    ...
    for name, shapes in us_states.items():
        sentiment = state_sentiments.get(name)
